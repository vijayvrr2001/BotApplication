...
{
    [Serializable]
    public class DrugsThatTreat : IEnumerable
    {
    ...
    [Serializable]
    public class DrugsListing
    {
        strBra
        public void setFieldValues(string strToSet)
        {
        ...
        public string drugs;
        public static IForm<DrugsListing> BuildForm()
        {
            OnCompletionAsyncDelegate<DrugsListing> processOrder = async (context, state) =>
            {
            ...
            return new FormBuilder<DrugsListing>()
                    .Message("Welcome to Indications checker from Swaas !")
                    .Field(new FieldReflector<DrugsListing>(nameof(drugs))
                            .SetType(null)
    ...
    {
        internal static IDialog<DrugsListing> MakeRootDialog()
        {
            return Chain.From(() => FormDialog.FromForm(DrugsListing.BuildForm));
        }
        ...
        public async Task<HttpResponseMessage> Post([FromBody]Activity activity)
        {
            ...
            if (activity.Type == ActivityTypes.Message)
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    HttpResponseMessage responseInternal = await client.GetAsync("/indexes/drugtest/docs?api-version=2015-02-28&search=(" + sbIndications.ToString() + ") &queryType=full&searchMode=all&$skip=5&$select=brandName,company");
                    if (responseInternal.IsSuccessStatusCode)
                    {
                        JavaScriptSerializer serializer = new JavaScriptSerializer();
                        Stream streamTask = await responseInternal.Content.ReadAsStreamAsync();
                        DrugsListing drgListing = new DrugsListing();
                        DrugsThatTreat dr = new DrugsThatTreat();
                            ...
                            foreach (KeyValuePair<string, object> entry in dictionary)
                            {
                                if (entry.Value is ArrayList)
                                {
                                    foreach (var item in (ICollection)entry.Value)
                                    {
                                        ...
                                        dr.Brands.Add(strBrandName + strCompName);
                                       
                                    }
                                    //Activity replyFinal = activity.CreateReply("Top drugs that treat the indication " + activity.Text + " are " + sb.ToString());
